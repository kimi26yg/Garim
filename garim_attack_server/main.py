...
class FaceSwapper:
    def __init__(self, model_path):
        ...
        # Optimized CoreML Configuration
        self.providers = [
    ...
    def load_models(self):
        try:
            print("[AI] Initializing FaceAnalysis (buffalo_l)...")
            self.face_evaluator = FaceAnalysis(name='buffalo_l', providers=self.providers)
            try:
                # Optimized for Cropped Faces: (640, 640) for high precision on small inputs
                ...
                print(f"[Warning] Failed to use GPU/CoreML for FaceAnalysis: {e}")
        ...
        try:
            print(f"[AI] Loading Swapper model from {self.model_path}...")
...
def attack_start(sid, data):
    ...
    # No cropping needed; client sends face crop
    # cropped_img = crop_to_9_16(raw_img) 
...
if __name__ == '__main__':
    print(f"[System] Starting Socket.IO server on port {PORT}...")
