...
Deepfake technology has made it trivially easy to swap someone's face onto a live video call — impersonating a friend, family member, or authority figure in real time. Existing security measures don't cover this: by the time you notice something is off, the damage is done. Garim Eye was built to close that gap — running on-device inference directly on the incoming video stream so the person on the receiving end gets an instant, continuous trust signal without relying on any server or third party.

Garim Eye v2 is a mobile application that detects deepfake threats in real-time video calls. Powered by a **Hybrid Inference Engine** (CNN + FFT Analysis), it provides users with immediate visual feedback on the authenticity of the video feed, ensuring secure and trustworthy communications.
...
2. **Installation**
   ...
   git clone https://github.com/kimi26yg/Garim.git
   cd Garim/garim_eye
...
딥페이크 기술은 이제 실시간 영상 통화에서 타인의 얼굴을 손쉽게 합성할 수 있는 수준까지 왔습니다. 지인, 가족, 혹은 권위 있는 인물로 위장한 공격이 가능한데도 기존 보안 수단은 이를 탐지하지 못합니다. 피해를 인지할 때는 이미 늦은 경우가 대부분입니다. Garim Eye는 이 공백을 메우기 위해 만들어졌습니다 — 서버나 외부 서비스 없이, 수신되는 영상 스트림에 대해 온디바이스 추론을 직접 실행해 상대방에게 실시간 신뢰 지표를 제공합니다.

Garim Eye v2는 실시간 영상 통화에서 딥페이크 위협을 탐지하도록 설계된 모바일 애플리케이션입니다. **하이브리드 추론 엔진** (CNN + FFT 분석)을 기반으로 구동되며, 영상의 진위 여부에 대한 즉각적인 시각적 피드백을 제공하여 안전하고 신뢰할 수 있는 커뮤니케이션을 보장합니다.
